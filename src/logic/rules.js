...
// All functions are pure — they receive (gameState, ui) or (ctx) as parameters.
// No module-level state imports.

import { rn, fiv, hasSubsetSum, isDoubled } from './constants.js'

// ── Build move context from reactive gameState + ui ──
export function buildMoveCtx(G, ui) {
  if (ui.selHandIdx === null) return null;
  const cp   = G.currentPlayer;
  const hand = G.hands[cp];
  const hc   = hand[ui.selHandIdx];
  const sel  = ui.selFloorIdxs.map(i => G.floor[i]);
  return {
    floor:        G.floor,
    floorVals:    G.floor.map(fiv),
    hand,
    handCard:     hc,
    handIdx:      ui.selHandIdx,
    selItems:     sel,
    selSum:       sel.reduce((s, it) => s + fiv(it), 0),
    selLoose:     sel.filter(it => it.type === 'card').map(it => it.card),
    selHouses:    sel.filter(it => it.type === 'house'),
    remainder:    hand.filter((_, i) => i !== ui.selHandIdx),
    team:         cp % 2,
    player:       cp,
    // thread through fields needed by rule functions
    isFirstTurn:  G.isFirstTurn,
    bidder:       G.bidder,
    bidValue:     G.bidValue,
    selFloorIdxs: ui.selFloorIdxs,
  };
}
...
export function rulePick(ctx) {
  ...
  if (ctx.isFirstTurn && ctx.player === ctx.bidder && ctx.handCard.rank !== ctx.bidValue)
    return { valid: false, reason: `First turn: can only pick with the bid card (${rn(ctx.bidValue)})` };
...
export function ruleBuildHouse(ctx) {
  ...
  if (ctx.isFirstTurn && ctx.player === ctx.bidder && ctx.handCard.rank === ctx.bidValue) {
    if (!ctx.remainder.some(c => c.rank === ctx.bidValue))
      return { valid: false, reason: `First turn: need a second ${rn(ctx.bidValue)} to build with it` };
  }
...
export function ruleBreakHouse(ctx) {
  ...
  if (isDoubled(h))  return { valid: false, reason: 'House is already doubled — only cementing applies' };
...
export function ruleThrow(ctx) {
  if (ctx.isFirstTurn && ctx.player === ctx.bidder && ctx.handCard.rank !== ctx.bidValue)
    return { valid: false, reason: `First turn: can only throw the bid card (${rn(ctx.bidValue)})` };
...
function buildStatusMsg(rules, ctx) {
  ...
  ctx.floor.forEach((it, i) => {
    if (fiv(it) === hc.rank && !ctx.selFloorIdxs.includes(i))
...
// ── Public entry point used by useGameState ──
export function computeActions(G, ui) {
  const ctx = buildMoveCtx(G, ui);
  if (!ctx) {
    return { pick: false, seep: false, build: false, buildVal: null,
             buildMerge: false, add: false, throw: false, msg: 'Select a card from your hand.' };
  }
